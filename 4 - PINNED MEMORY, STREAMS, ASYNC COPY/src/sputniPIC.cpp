...
int main(int argc, char **argv){
    ...
    */
        ...
        */
        ...
        long batch_size = 16384000; // Number of particles per batch
        ...
        // Create streams TODO HOW MANY
        long particles_per_stream = 409600;
        size_t num_streams = (batch_size + particles_per_stream - 1) / particles_per_stream;
    ...
    for (int cycle = param.first_cycle_n; cycle < (param.first_cycle_n + param.ncycles); cycle++) {
        ...
        for (int is=0; is < param.ns; is++) {
            ...
            std::cout << "***  MOVER with SUBCYCLYING "<< part[is].n_sub_cycles << " - species " << part[is].species_ID << " ***" << std::endl;
            for(int p = 0; p < part[is].nop; p += batch_size) {
                ...
                device_part_transfer(part[is], device_part, is, p, particles_to_move, streams, particles_per_stream, cudaMemcpyHostToDevice);
                mover_PC(device_part, device_field, device_grd, device_param, particles_to_move, part[is].species_ID, param.n_sub_cycles, part[is].qom, part[is].NiterMover, streams, particles_per_stream); // MODIFICATION: Function call has been modified.
                device_part_transfer(part[is], device_part, is, p, particles_to_move, streams, particles_per_stream, cudaMemcpyDeviceToHost);
            }
        }
        ...
        for (int is=0; is < param.ns; is++) {
            for(int p = 0; p < part[is].npmax; p += batch_size) {
                ...
                device_part_transfer(part[is], device_part, is, p, particles_to_move, streams, particles_per_stream, cudaMemcpyHostToDevice);
                interpP2G(device_part, device_ids[is], device_grd, particles_to_move, streams, particles_per_stream); // modifies [ids -> ]
                device_part_transfer(part[is], device_part, is, p, particles_to_move, streams, particles_per_stream, cudaMemcpyDeviceToHost);
            }
        }
        cudaDeviceSynchronize();
