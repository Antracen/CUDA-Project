...
#define TPB (16*20) // Threads per block
...
void particle_allocate(struct parameters* param, struct particles* part, int is)
{
    ...
    //////////////////////////////
    checkCudaErrors(cudaHostAlloc(&(part->x), npmax*sizeof(FPpart), cudaHostAllocDefault));
    checkCudaErrors(cudaHostAlloc(&(part->y), npmax*sizeof(FPpart), cudaHostAllocDefault));
    checkCudaErrors(cudaHostAlloc(&(part->z), npmax*sizeof(FPpart), cudaHostAllocDefault));
    checkCudaErrors(cudaHostAlloc(&(part->u), npmax*sizeof(FPpart), cudaHostAllocDefault));
    checkCudaErrors(cudaHostAlloc(&(part->v), npmax*sizeof(FPpart), cudaHostAllocDefault));
    checkCudaErrors(cudaHostAlloc(&(part->w), npmax*sizeof(FPpart), cudaHostAllocDefault));
    checkCudaErrors(cudaHostAlloc(&(part->q), npmax*sizeof(FPpart), cudaHostAllocDefault));    
}
/** deallocate */
...
/*
    The kernel makes every thread work on one single particle.
*/
__global__
void mover_PC_kernel(struct device_part_arrays part, struct EMfield* field, struct grid* grd, struct parameters *param, long nop, int n_sub_cycles, FPpart qom, int NiterMover, long offset) {

    // Find global thread index
    int part_index = blockIdx.x*blockDim.x + threadIdx.x;

    // Don't go outside array
    if(part_index >= nop) return;
    part_index += offset;
    ...
    /*
        I removed the loop going through every particle since each thread handles one particle.
    */
    // start subcycling
...
/** MODIFICATION: New particle mover */ // TODO COMMENT MORE
int mover_PC(struct device_part_arrays part, EMfield *field, grid* grd, parameters *param, long nop, int species_ID, int n_sub_cycles, FPpart qom, int NiterMover, cudaStream_t *streams, long particles_per_stream)
{
    int stream_index = 0;
    long moved_particles = 0;
    long stream_move_size;
    while(moved_particles < nop) {
        stream_move_size = particles_per_stream;
        if(stream_move_size + moved_particles > nop)
            stream_move_size = nop - moved_particles;

        int blocks = (stream_move_size + TPB - 1) / TPB;
        mover_PC_kernel<<<blocks, TPB, 0, streams[stream_index]>>>(part, field, grd, param, stream_move_size, n_sub_cycles, qom, NiterMover, moved_particles);
        cudaError_t err = cudaGetLastError();
        if(err != cudaSuccess) {
            std::cout << "Kernel mover_PC error: " << cudaGetErrorString(err) << std::endl;
            exit(0);
        }

        moved_particles += stream_move_size;
        stream_index++;
    }
...
__global__
void interpP2G_kernel(device_part_arrays part, device_ids_arrays ids, grid* grd, long nop, long offset) {
...
// TODO COMMENT WHEN DONE
void interpP2G(struct device_part_arrays part, struct device_ids_arrays ids, struct grid* grd, long nop, cudaStream_t *streams, long particles_per_stream) {

    int stream_index = 0;
    long interpolated_particles = 0;
    long stream_interp_size;
    while(interpolated_particles < nop) {
        stream_interp_size = particles_per_stream;
        if(interpolated_particles + stream_interp_size > nop)
            stream_interp_size = nop - interpolated_particles;

        int blocks = (stream_interp_size + TPB - 1) / TPB;
        interpP2G_kernel<<<blocks, TPB, 0, streams[stream_index]>>>(part, ids, grd, stream_interp_size, interpolated_particles);   
        cudaError_t err = cudaGetLastError();
        if(err != cudaSuccess) {
            std::cout << "Kernel mover_PC error: " << cudaGetErrorString(err) << std::endl;
            exit(0);
        }
        interpolated_particles += stream_interp_size;
        stream_index++;
    }
}